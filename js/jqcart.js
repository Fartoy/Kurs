  ...
  var actions = {
    ...
    openCart: function() {
      var subtotal = 0,
			cartHtml = '';
      cartData = actions.getStorage();
      orderPreview = '<p class="jqcart-cart-title">Cart </p><div class="jqcart-table-wrapper"><div class="jqcart-manage-order"><div class="jqcart-thead"><div>ID</div><div></div><div>Name pizza</div><div>Price</div><div>Count</div><div>Amount</div><div></div></div>';
      var key, sum = 0;
      for (key in cartData) {
        if (cartData.hasOwnProperty(key)) {
      ...
      orderPreview += '<div class="jqcart-subtotal">Total: <strong>' + subtotal + '</strong> ' + opts.currency + '</div>';
